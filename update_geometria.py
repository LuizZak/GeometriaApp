...
GEOMETRIA_REPO = "https://github.com/LuizZak/Geometria.git"
GEOMETRIA_TARGET_PATH = cwd_path("Sources", "Geometria")
...
def clone_geometria(tag_or_branch: str | None, base_folder: Path) -> Path:
    print("Cloning Geometria...")

    geometria_clone_path = str(path(base_folder, "geometria").absolute())

    clone_repo(tag_or_branch, GEOMETRIA_REPO, geometria_clone_path)

    return Path(geometria_clone_path)
...
def copy_geometria_files(clone_path: Path, target_path: Path):
    print("Copying over Geometria files...")

    copy_repo_files(clone_path.joinpath("Sources", "Geometria"), target_path)


def update_code(geometria_tag_or_branch: str | None, force: bool) -> int:
    ...
    # Clone
    geometria_clone_path = clone_geometria(geometria_tag_or_branch, temp_path)

    # Copy files
    copy_geometria_files(geometria_clone_path, GEOMETRIA_TARGET_PATH)
...
def main() -> int:
    def make_argparser() -> argparse.ArgumentParser:
        ...
        argparser.add_argument(
            ...
            "--geometria_tag",
            type=str,
            help="A tag or branch to clone from the Geometria repository. If not provided, defaults to latest commit of default branch.",
        )
        argparser.add_argument(
            "-f",
            "--force",
    ...
    return update_code(args.geometria_tag, args.force)
