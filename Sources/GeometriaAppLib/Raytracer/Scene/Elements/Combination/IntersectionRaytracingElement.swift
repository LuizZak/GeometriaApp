...
extension IntersectionRaytracingElement: RaytracingElement {
    ...
    public func raycast(query: RayQuery, results: inout SortedRayHits) {
        ...
        if t1Hits.isEmpty && !t1.contains(point: noIgnoreQuery.ray.start) {
            return
        }
    ...
    // TODO: Handle ray containment when the ray crosses the boundaries of both geometries but stays within the overall volume of the intersection
    @inlinable
