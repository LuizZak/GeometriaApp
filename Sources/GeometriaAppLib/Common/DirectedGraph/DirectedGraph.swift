...
public extension DirectedGraph {
    ...
    /// Returns `true` if the directed graph has one or more edges that connect
    /// the two given nodes directly.
    ///
    /// Alias for `areConnected(start:end:)`.
    @inlinable
    func hasEdge(from start: Node, to end: Node) -> Bool {
        areConnected(start: start, end: end)
    }
