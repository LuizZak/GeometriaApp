...
//        let batcher = LinearBatcher()
        ...
        rendererCoordinator?.stateDidChange.addListener(weakOwner: self) { [weak self] (_, change) in
...
//        ctx.strokeLine(p0: _mouseLocation - .init(x: 0, y: crosshairLength), p1: _mouseLocation + .init(x: 0, y: crosshairLength))
    ...
    func drawLabel(_ renderer: Blend2DRenderer, text: String, topLeft: BLPoint) {
